 ...
 *
 * Function:
 *     Gua_Status Lmtx_Process(Gua_Object *nn, Gua_Object *in, Gua_Object *out, Gua_Integer ni, Gua_Integer no, Gua_Integer af, Gua_Integer oaf, Gua_Integer of, Gua_Object *ofc)
 *
 * Description:
...
Gua_Status Lmtx_Process(Gua_Object *nn, Gua_Object *in, Gua_Object *out, Gua_Integer ni, Gua_Integer no, Gua_Integer af, Gua_Integer oaf, Gua_Integer of, Gua_Object *ofc)
{
    ...
    if (ofc != NULL) {
        mofc = (Gua_Matrix *)Gua_PObjectToMatrix(ofc);
    }
    ...
    if (mnn && min) {
        ...
        for (i = 0; i < no; i = i + 1) {
            if (of == LMTX_LINEAR_OUTPUT_FUNCTION) {
                Gua_RealToObject(oout[i], Gua_ObjectToReal(oofc[0]) * Gua_ObjectToReal(onn[(first_out + i) * dim_j + 0]) + Gua_ObjectToReal(oofc[1]));
            } else if (of == LMTX_STEP_OUTPUT_FUNCTION) {
                if (oaf == LMTX_LINEAR_ACTIVATION_FUNCTION) {
...
Gua_Status Lmtx_FunctionWrapper(void *nspace, Gua_Short argc, Gua_Object *argv, Gua_Object *object, Gua_String error)
{
     ...
     *
     * Function:
     *     lmtxProcess(nn, in, ni, no, af, oaf, of, c)
     *
     * Description:
     *      Process a pulse in an artificial neural network.
     *
     * Examples:
     *     output == lmtxProcess(lmtx, input, dim_in_j, dim_out_j, LMTX_SIGMOID_ACTIVATION_FUNCTION, LMTX_SIGMOID_ACTIVATION_FUNCTION, LMTX_STEP_OUTPUT_FUNCTION, c).
     */
        ...
        if (argc == 9) {
            if (Gua_ObjectType(argv[8]) != OBJECT_TYPE_MATRIX) {
                errMessage = (Gua_String) Gua_Alloc(sizeof(char) * MAX_ERROR_MSG_SIZE + 1);
                sprintf(errMessage, "%s %-.20s...\n", "illegal argument 8 for function", Gua_ObjectToString(argv[0]));
                strcat(error, errMessage);
                Gua_Free(errMessage);
        ...
        if (Lmtx_Process(&(argv[1]), &(argv[2]), object, Gua_ObjectToInteger(argv[3]), Gua_ObjectToInteger(argv[4]), af, oaf, of, ofc) != GUA_OK) {
            errMessage = (Gua_String) Gua_Alloc(sizeof(char) * MAX_ERROR_MSG_SIZE + 1);
            sprintf(errMessage, "%s %-.20s...\n", "illegal arguments for function", Gua_ObjectToString(argv[0]));
            strcat(error, errMessage);
            Gua_Free(errMessage);
...
Gua_Status Lmtx_Init(void *nspace, int argc, char *argv[], char **env, Gua_String error)
{
     ...
     *
     * Constant:
     *     LMTX_LINEAR_OUTPUT_FUNCTION
     *
     * Description:
     *     Use the step function on the neural network output.
     */
    Gua_IntegerToObject(object, LMTX_LINEAR_OUTPUT_FUNCTION);
    Gua_SetStoredObject(object);
    if (Gua_SetVariable((Gua_Namespace *)nspace, "LMTX_LINEAR_OUTPUT_FUNCTION", &object, SCOPE_GLOBAL) != GUA_OK) {
        errMessage = (Gua_String) Gua_Alloc(sizeof(char) * MAX_ERROR_MSG_SIZE + 1);
        sprintf(errMessage, "%s %-.20s...\n", "can't set variable", "LMTX_LINEAR_OUTPUT_FUNCTION");
        strcat(error, errMessage);
        Gua_Free(errMessage);
    }
    /**
